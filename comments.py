...
def Task_23(start,end,multi=[9999],plus=[0],inc=[-1],ex=[-1]):
	#Использование: print(Task_23(начальное значение, конечное значение, multi=[числа, на которые умножаются], plus=[которые прибавляются], inc=[обязательные в траекториии], ex=[исключаемые значения]))
	#multi, plus, inc и ex - опционально. Если нет таких условий, просто не пишете их, вызывая функцию (не передаете этот параметр, передаете только то, что нужно)
	...
	#Прибавить plus=[a,b,...], Умножить на multi=[a,b,...] и при этом траектория содержит число inc=[a,b,...] и не содержит ex=[a,b,...]
